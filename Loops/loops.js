...
for (var i_1 = 1; i_1 <= 10; i_1++) {
    var lesson = {
        ...
        status: i_1 % 2 === 1 //true if remainder is 1 otherwise false
    };
...
//Set a maximum value
var maxValue = 10; // You can change this value as needed
//Generate a random number
var randomNumber = Math.floor(Math.random() * maxValue) + 1;
console.log("Random Number is: ", randomNumber);
//Track the guess status
var guessCorrect = false; // Initially, no guess has been made, so set to false
//Simulate user guesses
var guesses = [1, 3, 5, 8, 10]; // Predefined guesses array
//Iterate over guesses using a while loop
var i = 0;
while (i < guesses.length && !guessCorrect) {
    var currentGuess = guesses[i];
    console.log("Guess #".concat(i + 1, ": ").concat(currentGuess)); // Using i + 1 for user-friendly display
...
var step = 10;
do {
    console.log("Counter: ".concat(counterValue));
    counterValue += step;
...
for (var myProp in myObj) {
    console.log("Property: ".concat(myProp, ", Values: ").concat(myObj[myProp]));
}
...
for (var i_3 = 0; i_3 < myArray.length; i_3++) {
    console.log("Index ".concat(i_3, ": ").concat(myArray[i_3]));
}
//By using for...of Loop print the output of myArray elements
console.log("\nBy using for...of Loop: ");
for (var _i = 0, myArray_1 = myArray; _i < myArray_1.length; _i++) {
    var myElements = myArray_1[_i];
